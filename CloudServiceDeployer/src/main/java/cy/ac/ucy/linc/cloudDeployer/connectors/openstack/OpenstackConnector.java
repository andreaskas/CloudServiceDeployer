...
public class OpenstackConnector implements ICloudConnector {
	...
	public OpenstackConnector(HashMap<String, String> params) {
		...
		this.computeAPI = builder.buildView(ComputeServiceContext.class)
				.getComputeService();
		this.novaAPI = builder.buildApi(NovaApi.class);
		this.zones = this.novaAPI.getConfiguredZones();

		this.networkAPI = ContextBuilder
	...
	public String createInstance(Map<String, String> params) {
		...
		CreateServerOptions serverOpts = CreateServerOptions.Builder.adminPass(
				"linc").networks(networks);
		serverOpts.keyPairName(keypair);
		serverOpts.securityGroupNames(securityGroup);

		System.out
				.println("OpenstackConnector>> received ADD instance request, with instance name: "
						+ instName);

		ServerApi serverAPI = this.novaAPI
				.getServerApiForZone(OpenstackConnector.DEFAULT_REGION);
		ServerCreated s = serverAPI.create(instName, imageID, flavorID,
				serverOpts);
	...
	public List<FlavorObj> getFlavorList(Map<String, String> params) {
		...
		FlavorApi zonesapi = this.novaAPI.getFlavorApiForZone(DEFAULT_REGION);
	...
	public List<NetworkObj> getNetworks() {
		...
		for (Network n : this.networkAPI
				.getNetworkApi(OpenstackConnector.DEFAULT_REGION).list()
				.concat()) {
			networklist.add(new NetworkObj(n.getName(), n.getId()));
	...
	public List<SecurityGroupsObj> getSecurityGroups() {
		...
		Optional<? extends SecurityGroupApi> securityGroupApi = this.novaAPI
				.getSecurityGroupExtensionForZone(DEFAULT_REGION);
